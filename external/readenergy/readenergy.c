...
inline uint32_t join_32bit_register(uint32_t *buffer, int index)
{
	uint32_t result = 0;
	result |= buffer[index];
	result |= (uint32_t)(buffer[index+1]) << 32;
	return result;
}
...
void emeter_read_measurements(struct emeter *this, struct reading *reading)
{
	...
	reading->sys_enm_ch0_sys = (double)join_32bit_register(buffer, BASE_INDEX+12) / SYS_ENM_CH0_SYS_SCALE;
	reading->sys_enm_ch1_a57 = (double)join_32bit_register(buffer, BASE_INDEX+14) / SYS_ENM_CH1_A57_SCALE;
	reading->sys_enm_ch0_a53 = (double)join_32bit_register(buffer, BASE_INDEX+16) / SYS_ENM_CH0_A53_SCALE;
	reading->sys_enm_ch0_gpu = (double)join_32bit_register(buffer, BASE_INDEX+18) / SYS_ENM_CH0_GPU_SCALE;
}
